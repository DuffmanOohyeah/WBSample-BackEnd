    ...
    console.log('hash:', hash);*/
		...
		if (pwdMatch) {
			...
			if (result[0].account_suspended) {
				rtnObj.data =
					result[0].suspended_reason ||
					`Account suspended; please contact ${req.app.get('app_name')}.`;
				allowLogin = false;
			} else {
				allowLogin = true;
			}
		...
		if (allowLogin) {
			...
			if (result[0].client_ids.length == 1) {
				// only one client associated; session it
			...
			await db2
				.collection('users')
				.updateOne({ _id: o_id }, { $set: { last_login_date: new Date() } });
