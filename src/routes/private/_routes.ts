...
router.get('/', (req: any, res: any) => {
  res.status(req.statusCode || 200)
    .json({zone: 'Entry to private endpoint'});
});
...
router.get('/setupAnalyticsDb/:setupPwd', async (req: any, res: any) => {
  if( req.params.setupPwd === config.appVars.setupPwd ){
    const doAnalyticsSetup: any = await setup_analytics_mongodb.doSetup(req, res);
    res.status(req.statusCode || 200)
      .json(doAnalyticsSetup);
  }else{
    res.status(400)
      .json({err: 'Authorisation denied.'});
  }
