...
const post: any = async (req: any) => {
	...
	try {
		...
		if (isValidEmail) {
			...
			if (result._id) {
				const domain: string = await emailUtils.primaryDomain(
					result.client_ids[0]
				);

				if (!result.email_validated) {
					rtnObj.data =
						'Your account needs confirmation; please check your inbox.';
					// TODO: send validation email
					if (domain.length) {
						// TODO: send nodemailer link
						let subject: string = `Please confirm your email with ${req.app.get(
							'app_name'
						)}`;
						let html: string = '<p>';
						html +=
							"In order to access the application, we'd appreciate if you could confirm your email address by clicking the link below:";
						...
						html += '</p>';
						/*const emailInfo: any = await*/ emailUtils.send(
							email,
							subject,
							html
						);
					...
					rtnObj.data = `Your account has been suspended; please contact ${req.app.get(
					...
					if (domain.length) {
						let subject: string = `Your password reset link from ${req.app.get(
							'app_name'
						)}`;
						let html: string = '<p>';
