...
const onSioConnection: any = () => {
  sio.on('connection', async (socket: Socket) => {
    ...
    const user = await UserService.addSocketInfoToUser({user_id:data.user_id, socket_id:socket.id,username:data.username});
    socket.join(data.room_id);
    await ParticipantService.AddUserToRoom(data.user_id,data.room_id,socket.id);
    if(user !== null){
      socket.emit('message',{username:user.username,text:"welcome to chat", time:Date.now()});
    }
    ...
    socket.on('disconnect', async () => {
      ...
      const participant = await ParticipantService.removeUserFromRoom(socket.id);
      const users = await ParticipantService.gelAllUsersByRoomId(data.room_id);
      sio.to(data.room_id).emit('roomUsers',{room:"",users:users});
    });
...
app.get('/api/participant',async(_,res:Response)=>{
   const paticipants = await Pariticipant.find().populate('room').populate('user').exec();
...
app.use((req: any, res: any) => {
  res.status(404)
      .send('Sorry we\'ve looked, but can\'t find that endpoint.')
  ;
});
...
const assignPorts: any = () => {
    ...
    console.log(`Worker PID ${process.pid} says ... the ${process.env.NODE_ENV} server is running on port ${process.env.PORT}.`);
