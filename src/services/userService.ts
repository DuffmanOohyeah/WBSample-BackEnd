...
export const addSocketInfoToUser = async (
	...
	return User.findOneAndUpdate(
		...
		{ $push: { socket_id: socketInfo.socket_id } }
	);
};

export const removeSocketInfoFromUser = async (
	socketId: string
): Promise<UserDocument | null> => {
	return User.findOneAndUpdate(
		{ socket_id: { $in: [socketId] } },
		{ $pull: { socket_id: socketId } },
		{ multi: true, new: true }
	);
};
