...
export const createMessageInRoom = async(userId:string,roomId:string,message:string):Promise<MessageDoc> => {
        const user = await UserService.findUserByUserID(userId);
        const room = await RoomService.getRoomById(roomId);
        return Message.create({user:user,room:room,message:message,created_at: new Date(Date.now())})
}
