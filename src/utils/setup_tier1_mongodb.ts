	...
	try {
		...
		if (!arr.length) {
			const insert: any = await collection.insertMany([
				...
				{
					setting: 'wbsample_company_name',
					value: config.appVars.wbSampleCompanyName,
				},
				{
					setting: 'wbsample_from_email',
					value: config.appVars.wbSampleFromEmail,
				},
			]);
			if (insert.result && insert.result.ok) {
				rtnBln = true;
	...
	try {
		...
		if (!arr.length) {
			...
			const insert: any = await collection.insertOne({
				...
				password_hash:
					'$2a$10$FzXZhi/vZYFhf2g0YAdQ1e3le1i1fVkHYy/Fwi65SIwW9UHgwUgou',
